...
# Redis settings (non-standard port)
...
# -----------------------------------------------------------------------------
log_info() {
    echo "[INFO] $(date '+%Y-%m-%d %H:%M:%S') [system-status] $1" | tee -a "$LOG_FILE"
}
...
log_success() {
    echo "[SUCCESS] $(date '+%Y-%m-%d %H:%M:%S') [system-status] $1" | tee -a "$LOG_FILE"
}

log_warn() {
    echo "[WARN] $(date '+%Y-%m-%d %H:%M:%S') [system-status] $1" | tee -a "$LOG_FILE"
}
...
show_container_status() {
    ...
    while IFS=$'\t' read -r name status; do
        ...
        # Extract state from status (e.g., "Up 2 hours (healthy)")
        local state="running"
        local health="N/A"
...
show_endpoint_status() {
    ...
    local pg_status
    pg_status=$(docker exec n8n-postgres pg_isready -U n8n 2>/dev/null)
    if [[ $? -eq 0 ]]; then
