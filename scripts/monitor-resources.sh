...
check_memory() {
	...
	local mem_info
	if ! mem_info=$(free -b 2>/dev/null); then
...
check_disk() {
	...
	if [[ -n "$project_disk" ]]; then
		...
		local total_size used_size avail_size
		total_size=$(echo "$project_disk" | awk '{print $2}')
		used_size=$(echo "$project_disk" | awk '{print $3}')
		avail_size=$(echo "$project_disk" | awk '{print $4}')
